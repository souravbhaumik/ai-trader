...
def _get_db_conn():
    """Open a synchronous psycopg connection (Celery worker is sync)."""
    import psycopg2  # noqa: import-error

    from urllib.parse import urlparse, unquote
    from app.core.config import settings

    # Build sync DSN from settings
                ...
                for sym in batch:
                    try:
                        if isinstance(close_df, pd.Series):
                            c = close_df; o = open_df; h = high_df; lo = low_df; v = volume_df
                        else:
                        ...
                        for ts, close_val in c.dropna().items():
                            rows_to_insert.append((
                                ...
                                float(o.get(ts, close_val)),
                                float(h.get(ts, close_val)),
                                float(lo.get(ts, close_val)),
                                float(close_val),
                                int(v.get(ts, 0) or 0),
                                "yfinance",
                            ))
