...
- **Phase 11: Institutional Alpha (The "Delivery" Engine)**
    ...
    - Meta-Labeling: Use a secondary "Filter" model to discard low-confidence BUY/SELL signals.
    - Volatility-Adjusted Profit Targets: Dynamic target prices based on ATR.
...
Signals are only emitted if: `Primary_Direction == BUY` AND `Meta_Confidence > 0.65`.
